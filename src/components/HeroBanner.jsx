...
export default function HeroBanner({ movie, onPlayWeb, onPlayNative, onOpenDetails }) {
  ...
  return (
    <div className="relative w-full h-[80vh] md:h-[88vh] overflow-hidden bg-black flex items-end justify-start px-6 md:px-16 pb-12 md:pb-16">
      ...
      {/* FLOATING CARD CONTAINER (Positioned on the Left Side - Exact match to Image 2) */}
      <div className="relative z-20 w-full max-w-3xl lg:max-w-4xl bg-[#1c1e24]/90 backdrop-blur-2xl border border-white/15 rounded-[32px] p-6 sm:p-8 shadow-[0_30px_70px_rgba(0,0,0,0.85)] animate-fade-in flex flex-col md:flex-row items-center md:items-start gap-6 md:gap-8">
        ...
        <div className="flex flex-col items-center shrink-0">
          ...
          <span className="text-[11px] sm:text-xs font-black tracking-widest text-neutral-200 uppercase mt-3.5 text-center">
            DISCO LOCAL
          </span>
        </div>

        {/* Right Column: Title, Subtitle, Action Buttons & Info */}
        <div className="flex-1 text-left flex flex-col justify-between self-stretch pt-2">
          <div>
            <h1 className="text-3xl sm:text-4xl md:text-5xl font-black text-white leading-tight tracking-tight mb-2.5">
              {movie.title}
            </h1>
            <p className="text-sm sm:text-base text-neutral-300 font-medium leading-relaxed mb-6">
              {movie.isSeries ? `Série animada ocidental: ${movie.title}` : (movie.overview || movie.title)}
            </p>
          </div>
          ...
          <div className="mt-auto">
            ...
            <div className="flex flex-wrap items-center gap-4 sm:gap-5 mb-3.5">
              <button
                ...
                className="bg-gradient-to-r from-[#E50914] to-[#B20710] hover:from-[#f6121d] hover:to-[#c80812] text-white px-7 sm:px-8 py-3.5 rounded-2xl flex items-center gap-3.5 shadow-xl shadow-[#E50914]/40 border border-red-500/40 cursor-pointer transition-all duration-300 hover:scale-105 active:scale-95 whitespace-nowrap min-w-[220px] sm:min-w-[250px]"
              >
                ...
                <div className="text-left pr-2">
                  ...
                  <span className="block text-[11px] text-white/80 font-normal mt-0.5">Reproduzir no navegador</span>
                </div>
              </button>

              <button
                onClick={() => onPlayNative(movie)}
                className="bg-white/10 hover:bg-white/20 border border-white/20 text-white px-7 sm:px-8 py-3.5 rounded-2xl flex items-center gap-3.5 backdrop-blur-md cursor-pointer transition-all duration-300 hover:scale-105 active:scale-95 whitespace-nowrap min-w-[220px] sm:min-w-[250px]"
              >
                ...
                <div className="text-left pr-2">
                  ...
                  <span className="block text-[11px] text-neutral-300 font-normal mt-0.5">Assistir no seu player local</span>
                </div>
              </button>
            </div>
            ...
            <button
              ...
              className="w-8 h-8 rounded-full border border-white/30 text-neutral-300 hover:text-white hover:border-white transition-all cursor-pointer flex items-center justify-center hover:scale-110"
